...
This account is for **infra setup** — running `snowflake/setup.sql`, `streams.sql`, and `tasks.sql` — not for the ingestion pipeline itself (that uses `CRYPTO_PIPELINE_ROLE`, created *by* this account). Key-pair authentication is used instead of a password so nothing secret ever needs to be typed into a SQL client or stored as a plaintext password.
...
```sql
CREATE USER IF NOT EXISTS CRYPTO_DEV_SVC
  RSA_PUBLIC_KEY = '<contents of rsa_key.pub, header/footer/newlines removed>'
  DEFAULT_ROLE = SYSADMIN
  DEFAULT_WAREHOUSE = CRYPTO_WH
  MUST_CHANGE_PASSWORD = FALSE;

GRANT ROLE SYSADMIN  TO USER CRYPTO_DEV_SVC;  -- create/manage warehouses, databases, schemas, tables
GRANT ROLE USERADMIN TO USER CRYPTO_DEV_SVC;  -- create/manage CRYPTO_PIPELINE_ROLE
```

Snowflake splits these privileges across two built-in roles — `SYSADMIN` cannot run `CREATE ROLE` on its own, and `USERADMIN` cannot create warehouses/databases — so the dev account needs both. `snowflake/setup.sql` switches between them itself (`USE ROLE SYSADMIN` / `USE ROLE USERADMIN`) as it creates each kind of object.

### 1.3 Run the setup scripts

Using [SnowSQL](https://docs.snowflake.com/en/user-guide/snowsql) (or paste each file into a Snowsight worksheet while authenticated as `CRYPTO_DEV_SVC`):

```cmd
:: Windows cmd
snowsql -a <account_identifier> -u CRYPTO_DEV_SVC ^
...
This creates `CRYPTO_WH`, all four databases (`CRYPTO_DB`, `DEV_DB`, `QA_DB`, `PROD_DB`), their schemas, the raw landing table, and `CRYPTO_PIPELINE_ROLE` — the role the ingestion script and dbt use day-to-day. Grant `CRYPTO_PIPELINE_ROLE` to whatever user your ingestion script / CI authenticates as:

```sql
GRANT ROLE CRYPTO_PIPELINE_ROLE TO USER <your_ingestion_user>;
```

### 1.4 Wire the service account into GitHub Actions (`infra-deploy.yml`)

`infra-deploy.yml` re-runs `setup.sql`/`streams.sql`/`tasks.sql` on every merge to `main` that touches `de-pipeline/snowflake/**`, authenticating as `CRYPTO_DEV_SVC`. Add two repository secrets (**Settings → Secrets and variables → Actions**):

| Secret | Value |
|---|---|
| `SNOWFLAKE_DEV_SVC_USER` | `CRYPTO_DEV_SVC` |
| `SNOWFLAKE_DEV_SVC_PRIVATE_KEY` | the full contents of `.keys/rsa_key.p8`, including the `-----BEGIN/END PRIVATE KEY-----` lines |

`SNOWFLAKE_ACCOUNT` is shared with `de-deploy.yml` (Step 5) and only needs to be added once. All three scripts are written to be safe to re-run (`CREATE ... IF NOT EXISTS`), so repeat deploys are idempotent — no destructive drops or offset resets.
